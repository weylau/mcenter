...
func (app *App) Run() {
	...
	logPath := flag.String("log", "", "Log file directory?")
	flag.Parse()
	applicationDir := ""
	if *wdPath != "" {
	...
	defaultQueuesConfPath := fmt.Sprintf("%s/queues.json", applicationDir)
	defaultConfPath := fmt.Sprintf("%s/config.ini", applicationDir)
	...
	fmt.Println("config:", sysConfig)
	if err != nil {
		panic("加载系统配置失败:" + *confPath)
	}
	app.sysConfig = sysConfig
	allQueues, err := config.GetQuenus(*queuesPath)
	if err != nil {
...
func (app *App) Receive() {
	...
	for _, queue := range app.queues {
		for i := 0; i < app.sysConfig.ReceiveNum; i++ {
			wg.Add(1)
			go func(queue *config.QueueConfig, exit chan struct{}) {
				defer wg.Done()
			RECONNECT:
				for {
					_, channel, err := util.SetupChannel(app.sysConfig.AmqpHost)
