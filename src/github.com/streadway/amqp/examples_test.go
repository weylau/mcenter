...
func ExampleConfig_timeout() {
	...
	conn, err := amqp.DialConfig("queue:///", amqp.Config{
...
func ExampleChannel_Confirm_bridge() {
	...
	source, err := amqp.Dial("queue://source/")
	if err != nil {
	...
	destination, err := amqp.Dial("queue://destination/")
	if err != nil {
...
func ExampleChannel_Consume() {
	...
	conn, err := amqp.Dial("queue://guest:guest@localhost:5672/")
	if err != nil {
